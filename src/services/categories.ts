...
export async function createCategory(input: Pick<Category, 'name' | 'description' | 'image_url'>) { const { data, error } = await supabase.from('categories').insert({ ...input, is_active: true }).select().single(); return assertData(data as Category | null, error, 'Unable to create category.'); }

...
export async function getProduct(id: string) { const { data, error } = await supabase.from('products').select('*').eq('id', id).single(); return assertData(data as Product | null, error, 'Unable to load product.'); }
